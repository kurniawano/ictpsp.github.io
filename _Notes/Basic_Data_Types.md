...
It is important to ask the question, "what kind of data is this?". One of the challenges **for** novice programmers is that Python does not require you to declare the *data type*. So many novice programmers do not think **about** this question, "what kind of data is this?". However, it is essential in every part of the problem-solving steps, to continue asking the question, "what kind of data are we dealing with?"
...
- `int`: whole numbers
- `float`: decimal numbers represented as floating **points**
...
What is the difference between the two codes above? In the first code, we print the world's labels 1, 2, and 3 as part of a single `string` data. On the other hand, in the second code, we print two types of data. The first data `"Hello World no. "` is a `string` data type. The second piece of data, however, is not. The data that is displayed on the screen, i.e., `1`, is an `int` data type. The comma (`,`) in between the two pieces of data is just to separate the different objects to be displayed.
