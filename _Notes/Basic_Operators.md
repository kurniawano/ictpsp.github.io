...
But this is not the value that we expected. We expected the speed to be about 12.8 km/h. The reason is that the units are wrong. This is why it is useful to print the computed value of our computation. Though there are no longer syntax errors, our code may not produce the correct output. To convert to km/h, we can add the conversion calculation.
